...
class JavaVM:
    ...
    def _handlers(self) -> dict[JavaVMFunction, Callable[[], "int | None"]]:
        return {
    ...
    def _destroy_java_vm(self) -> int | None:
        return JNIReturnCode.JNI_OK

    def _attach_current_thread(self) -> int | None:
        self.emu.mem.write_ptr(self.emu.arg(1), self.emu.jni.pointer)  # *env = JNIEnv*
        return JNIReturnCode.JNI_OK

    def _detach_current_thread(self) -> int | None:
        return JNIReturnCode.JNI_OK

    def _get_env(self) -> int | None:
        self.emu.mem.write_ptr(self.emu.arg(1), self.emu.jni.pointer)  # *env = JNIEnv*
        self.log.jni("GetEnv => %#x (JNI_OK)", self.emu.jni.pointer)
        return JNIReturnCode.JNI_OK

    def _attach_current_thread_as_daemon(self) -> int | None:
