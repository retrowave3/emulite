...
@dataclass
class DlPhdrInfo64(PackedStruct):
    SIZE = 64
    ...
    def pack(self) -> bytes:
        ...
        struct.pack_into("<Q", buf, 0, self.addr)
        struct.pack_into("<Q", buf, 8, self.name)
        struct.pack_into("<Q", buf, 16, self.phdr)
        struct.pack_into("<H", buf, 24, self.phnum & 0xFFFF)
        return bytes(buf)
