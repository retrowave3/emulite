...
from emulite.android.structs.dl_info32 import DlInfo32
from emulite.android.structs.dl_phdr_info32 import DlPhdrInfo32
...
class AndroidLibcHooks32:
    _MAIN_HANDLE = 0x1
    _RTLD_DEFAULT = 0
    _RTLD_NEXT = 0xFFFFFFFF
    _PROP_VALUE_MAX = 92
    _LOG_LEVELS = {2: "V", 3: "D", 4: "I", 5: "W", 6: "E", 7: "F"}

    def __init__(self, emu: "AndroidEmulator32"):
        self._emu = emu
    ...
    def _malloc(self, emu: "AndroidEmulator32") -> int:
        size = emu.arg(0)
        addr = self.heap.malloc(size)
        return addr

    def _calloc(self, emu: "AndroidEmulator32") -> int:
        count, size = emu.arg(0), emu.arg(1)
        addr = self.heap.calloc(count, size)
        emu.log.libc_call("calloc", f"{count}, {size}", addr)
        return addr

    def _realloc(self, emu: "AndroidEmulator32") -> int:
        ptr, size = emu.arg(0), emu.arg(1)
        addr = self.heap.realloc(ptr, size)
        emu.log.libc_call("realloc", f"{ptr:#x}, {size}", addr)
        return addr

    def _free(self, emu: "AndroidEmulator32") -> None:
        ptr = emu.arg(0)
        self.heap.free(ptr)
        return None

    def _memalign(self, emu: "AndroidEmulator32") -> int:
        alignment, size = emu.arg(0), emu.arg(1)
        addr = self.heap.memalign(alignment, size)
        emu.log.libc_call("memalign", f"{alignment}, {size}", addr)
        return addr

    def _posix_memalign(self, emu: "AndroidEmulator32") -> int:
        ...
        if alignment < 4 or (alignment & (alignment - 1)):
            ...
            return Errno.EINVAL
        addr = self.heap.memalign(alignment, size)
        emu.mem.write_u32(memptr, addr)
        emu.log.libc_call("posix_memalign", f"align={alignment}, size={size}", addr)
        return 0

    def _aligned_alloc(self, emu: "AndroidEmulator32") -> int:
        alignment, size = emu.arg(0), emu.arg(1)
    ...
    def _malloc_usable_size(self, emu: "AndroidEmulator32") -> int:
        ptr = emu.arg(0)
        size = self.heap.usable_size(ptr)
        emu.log.libc_call("malloc_usable_size", f"{ptr:#x}", size)
        return size

    def _dlopen(self, emu: "AndroidEmulator32") -> int:
        name_ptr = emu.arg(0)
        if name_ptr == 0:
    ...
    def _dlsym(self, emu: "AndroidEmulator32") -> int:
    ...
    def _lookup_scoped(self, emu: "AndroidEmulator32", module: NativeModule, name: str, seen: set[str]) -> int:
    ...
    def _dladdr(self, emu: "AndroidEmulator32") -> int:
        ...
        dlinfo = DlInfo32(fname=self._intern_cstr(emu.vfs.device_path(module)), fbase=module.base, sname=self._intern_cstr(nearest[0]) if nearest else 0, saddr=nearest[1] if nearest else 0)
        dlinfo.write_to(emu.mem, info_ptr)
        emu.log.libc("dladdr(%#x) => 1 (%s in %s)", addr, nearest[0] if nearest else "?", module.name)
        return 1

    def _dl_iterate_phdr(self, emu: "AndroidEmulator32") -> "int | None":
        callback, data = emu.arg(0), emu.arg(1)
        # The main executable (head), the dynamic linker (tail), and the synthetic shim libs bracket the
        # loaded modules — what real bionic reports, and consistent with the r_debug link_map chain.
        ld = emu.linker_debug
        ...
        while len(self._phdr_info_pool) <= depth:
            self._phdr_info_pool.append(emu.mem.mmap(DlPhdrInfo32.SIZE, label="dl_phdr_info"))
    ...
    def _dl_iterate_invoke(self, emu: "AndroidEmulator32", state: dict) -> None:
        module = state["modules"][state["index"]]
        info = state["info"]
        path = getattr(module, "device_path", None) or emu.vfs.device_path(module)  # synthetic images carry their own
        dlphdrinfo = DlPhdrInfo32(addr=module.base, name=self._intern_cstr(path), phdr=module.phdr_addr, phnum=module.phnum)
        dlphdrinfo.write_to(emu.mem, info)
        emu.set_arg(0, info)
        emu.set_arg(1, DlPhdrInfo32.SIZE)
        emu.set_arg(2, state["data"])
        emu.lr = self._phdr_next_slot
        emu.pc = state["callback"]

    def _dl_iterate_step(self, emu: "AndroidEmulator32") -> None:
    ...
    def _system_property_foreach(self, emu: "AndroidEmulator32") -> "int | None":
    ...
    def _prop_foreach_invoke(self, emu: "AndroidEmulator32", state: dict) -> None:
        emu.set_arg(0, state["infos"][state["index"]])  # const prop_info*
        emu.set_arg(1, state["cookie"])
        emu.lr = self._prop_foreach_slot
        emu.pc = state["callback"]

    def _prop_foreach_step(self, emu: "AndroidEmulator32") -> None:
    ...
    def _dlclose(self, emu: "AndroidEmulator32") -> int:
        emu.log.libc("dlclose(%#x) => 0 (kept loaded)", emu.arg(0))
        return 0

    def _dlerror_get(self, emu: "AndroidEmulator32") -> int:
    ...
    def _getauxval(self, emu: "AndroidEmulator32") -> int:
        at_type = emu.arg(0)
        value = emu.auxv.get(at_type, 0)
        emu.log.libc_call("getauxval", str(at_type), value)
        return value

    def _getenv(self, emu: "AndroidEmulator32") -> int:
    ...
    def _prop_value_bytes(value: str) -> bytes:
        raw = value.encode("utf-8")[: AndroidLibcHooks32._PROP_VALUE_MAX - 1]
        return raw.decode("utf-8", "ignore").encode("utf-8")

    def _system_property_get(self, emu: "AndroidEmulator32") -> int:
    ...
    def _system_property_find(self, emu: "AndroidEmulator32") -> int:
        name = emu.mem.read_cstr(emu.arg(0))
        info = emu.device.find(name)
        emu.log.libc("__system_property_find(%r) => %#x", name, info)
        return info

    def _system_property_read(self, emu: "AndroidEmulator32") -> int:
        info, name_out, value_out = emu.arg(0), emu.arg(1), emu.arg(2)
        found = emu.device.name_value(info)
        if found is None:
            emu.log.libc("__system_property_read(info=%#x) => 0 (not found)", info)
            return 0
        name, value = found
    ...
    def _android_log_vprint(self, emu: "AndroidEmulator32") -> int:
        priority, tag_ptr, fmt_ptr, ap = emu.arg(0), emu.arg(1), emu.arg(2), emu.arg(3)
        tag = emu.mem.read_cstr(tag_ptr) if tag_ptr else "?"
        message = cformat.VaListArgs32(emu, ap).format(emu.mem.read_cstr(fmt_ptr)) if fmt_ptr else ""
        return self._log_record(emu, priority, tag, message)

    def _android_log_buf_write(self, emu: "AndroidEmulator32") -> int:
        priority, tag_ptr, text_ptr = emu.arg(1), emu.arg(2), emu.arg(3)
        tag = emu.mem.read_cstr(tag_ptr) if tag_ptr else "?"
        text = emu.mem.read_cstr(text_ptr) if text_ptr else ""
        return self._log_record(emu, priority, tag, text)

    def _android_log_buf_print(self, emu: "AndroidEmulator32") -> int:
        priority, tag_ptr, fmt_ptr = emu.arg(1), emu.arg(2), emu.arg(3)
        tag = emu.mem.read_cstr(tag_ptr) if tag_ptr else "?"
        message = cformat.RegisterArgs32(emu, 4).format(emu.mem.read_cstr(fmt_ptr)) if fmt_ptr else ""
        return self._log_record(emu, priority, tag, message)

    def _android_log_is_loggable(self, emu: "AndroidEmulator32") -> int:
        emu.log.libc("__android_log_is_loggable => 1")
        return 1

    def _android_log_assert(self, emu: "AndroidEmulator32") -> int:
        cond_ptr, tag_ptr, fmt_ptr = emu.arg(0), emu.arg(1), emu.arg(2)
        cond = emu.mem.read_cstr(cond_ptr) if cond_ptr else "null"
        tag = emu.mem.read_cstr(tag_ptr) if tag_ptr else "?"
        fmt = cformat.RegisterArgs32(emu, 3).format(emu.mem.read_cstr(fmt_ptr)) if fmt_ptr else ""
        emu.log.crash("__android_log_assert [%s] %s: %s", tag, cond, fmt)
        raise EmulatorCrashed(f"__android_log_assert: [{tag}] {cond}: {fmt}")

    def _emit_formatted(self, emu: "AndroidEmulator32", buf: int, size: int, text: str, name: str) -> int:
    ...
    def _snprintf(self, emu: "AndroidEmulator32") -> int:
        buf, size, fmt = emu.arg(0), emu.arg(1), emu.arg(2)
        text = cformat.RegisterArgs32(emu, 3).format(emu.mem.read_cstr(fmt))
        return self._emit_formatted(emu, buf, size, text, "snprintf")

    def _sprintf(self, emu: "AndroidEmulator32") -> int:
        buf, fmt = emu.arg(0), emu.arg(1)
        text = cformat.RegisterArgs32(emu, 2).format(emu.mem.read_cstr(fmt))
        return self._emit_formatted(emu, buf, 1 << 62, text, "sprintf")

    def _vsnprintf(self, emu: "AndroidEmulator32") -> int:
        buf, size, fmt, ap = emu.arg(0), emu.arg(1), emu.arg(2), emu.arg(3)
        text = cformat.VaListArgs32(emu, ap).format(emu.mem.read_cstr(fmt))
        return self._emit_formatted(emu, buf, size, text, "vsnprintf")

    def _vsprintf(self, emu: "AndroidEmulator32") -> int:
        buf, fmt, ap = emu.arg(0), emu.arg(1), emu.arg(2)
        text = cformat.VaListArgs32(emu, ap).format(emu.mem.read_cstr(fmt))
        return self._emit_formatted(emu, buf, 1 << 62, text, "vsprintf")

    def _printf(self, emu: "AndroidEmulator32") -> int:
        text = cformat.RegisterArgs32(emu, 1).format(emu.mem.read_cstr(emu.arg(0)))
        sys.stdout.write(text)
        emu.log.libc("printf => %r", text)
        return len(text.encode("utf-8"))

    def _fprintf(self, emu: "AndroidEmulator32") -> int:
        text = cformat.RegisterArgs32(emu, 2).format(emu.mem.read_cstr(emu.arg(1)))
        sys.stdout.write(text)
        emu.log.libc("fprintf => %r", text)
        return len(text.encode("utf-8"))

    def _puts(self, emu: "AndroidEmulator32") -> int:
        text = emu.mem.read_cstr(emu.arg(0))
        sys.stdout.write(text + "\n")
        emu.log.libc("puts => %r", text)
        return len(text) + 1

    def _cxa_atexit(self, emu: "AndroidEmulator32") -> int:
        emu.log.libc("__cxa_atexit(fn=%#x) => 0", emu.arg(0))
        return 0

    def _cxa_finalize(self, emu: "AndroidEmulator32") -> int:
        emu.log.libc("__cxa_finalize(%#x) => 0", emu.arg(0))
        return 0

    def _register_atfork(self, emu: "AndroidEmulator32") -> int:
        emu.log.libc("__register_atfork() => 0 (no fork under emulation)")
        return 0

    def _stack_chk_fail(self, emu: "AndroidEmulator32") -> int:
        lr = emu.lr
        emu.log.crash("__stack_chk_fail: stack smashing detected (LR=%#x)", lr)
        raise EmulatorCrashed(f"__stack_chk_fail at LR={lr:#x}")

    def _errno(self, emu: "AndroidEmulator32") -> int:
        addr = emu.mem.errno_addr
        emu.log.libc("__errno() => %#x", addr)
        return addr

    def _abort(self, emu: "AndroidEmulator32") -> int:
        lr = emu.lr
