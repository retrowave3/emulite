...
from emulite.android.java.value_conversion import as_bytearray, as_bytes, as_float, as_int
...
class JavaByteBuffer(JavaObject):
    ...
    def wrap(array: object, *rest: object) -> JavaByteBuffer:
        buffer = JavaByteBuffer(as_bytearray(array))
        if len(rest) == 2:
            buffer._pos, buffer._limit = as_int(rest[0]), as_int(rest[0]) + as_int(rest[1])
        return buffer
    ...
    def get(self, *args: object) -> object:
        ...
        dst = as_bytearray(args[0])
        off = as_int(args[1]) if len(args) >= 3 else 0
        length = as_int(args[2]) if len(args) >= 3 else len(dst)
    ...
    def put(self, *args: object) -> JavaByteBuffer:
        ...
        if isinstance(source, JavaByteBuffer):
            ...
            data = as_bytes(source)
        if len(args) >= 3:
            data = data[as_int(args[1]) : as_int(args[1]) + as_int(args[2])]
        self._buf[self._pos : self._pos + len(data)] = data
        self._pos += len(data)
        return self

    def _get_num(self, fmt: str, size: int, args: tuple[object, ...]) -> int | float:
        if args:
            index = as_int(args[0])
        else:
    ...
    def _put_num(self, fmt: str, size: int, args: tuple[object, ...]) -> JavaByteBuffer:
        if len(args) == 2:
            index, value = as_int(args[0]), args[1]
        else:
            index, value = self._pos, args[0]
            self._pos += size
        if fmt in "fd":
            packed = struct.pack(self._order + fmt, as_float(value))
        else:
            packed = struct.pack(self._order + fmt.upper(), as_int(value) & ((1 << (size * 8)) - 1))
        self._buf[index : index + size] = packed
        return self

    def getShort(self, *args: object) -> int:
        return int(self._get_num("h", 2, args))

    def putShort(self, *args: object) -> JavaByteBuffer:
        return self._put_num("h", 2, args)

    def getChar(self, *args: object) -> int:
        return int(self._get_num("H", 2, args))

    def putChar(self, *args: object) -> JavaByteBuffer:
        return self._put_num("H", 2, args)

    def getInt(self, *args: object) -> int:
        return int(self._get_num("i", 4, args))

    def putInt(self, *args: object) -> JavaByteBuffer:
        return self._put_num("i", 4, args)

    def getLong(self, *args: object) -> int:
        return int(self._get_num("q", 8, args))

    def putLong(self, *args: object) -> JavaByteBuffer:
        return self._put_num("q", 8, args)

    def getFloat(self, *args: object) -> float:
        return float(self._get_num("f", 4, args))

    def putFloat(self, *args: object) -> JavaByteBuffer:
        return self._put_num("f", 4, args)

    def getDouble(self, *args: object) -> float:
        return float(self._get_num("d", 8, args))

    def putDouble(self, *args: object) -> JavaByteBuffer:
        return self._put_num("d", 8, args)

    def position(self, *args: object) -> object:
        if args:
            self._pos = as_int(args[0])
            return self
        return self._pos

    def limit(self, *args: object) -> object:
        if args:
            self._limit = as_int(args[0])
            return self
        return self._limit
