...
from emulite.android.java.value_conversion import as_float, as_int, as_text, unwrap


class JavaString(JavaObject[str]):
    ...
    @staticmethod
    def _apply_format(template: str, args: list[object]) -> str:
        counter = [0]
        ...
        def render(match: re.Match[str]) -> str:
            ...
            raw = unwrap(arg) if not isinstance(unwrap(arg), list) else arg
            if lower == "b":
            ...
            if lower == "s":
                ...
                return (spec + "s") % text
            if lower in "dox":
                return (spec + conversion) % as_int(raw)
            if lower in "feg":
                return (spec + conversion) % as_float(raw)
            if lower == "c":
    ...
    @classmethod
    def jni_construct(cls, args: list[object]) -> JavaString:
        if not args:
            return cls("")
        arg = args[0]
        if isinstance(arg, JavaObject):
            ...
            if isinstance(payload, (bytes, bytearray)):
                ...
                charset = as_text(args[1]) if len(args) > 1 else "utf-8"
    ...
    def replace(self, old: object, new: object) -> JavaString:
        if isinstance(old, int):
            return JavaString(self._s().replace(chr(old), chr(as_int(new))))
