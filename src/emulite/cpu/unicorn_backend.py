...
class UnicornBackend(Backend):
    """Unicorn-backed CPU implementation with explicit closed-state handling."""

    _HOOK_IDS: ClassVar[dict[HookType, int]] = {
    ...
    _UC: ClassVar[dict[CpuArch, tuple[int, int]]] = {CpuArch.ARM: (UC_ARCH_ARM, UC_MODE_ARM), CpuArch.ARM64: (UC_ARCH_ARM64, UC_MODE_ARM)}

    def __init__(self, cpu_arch: CpuArch = CpuArch.ARM64) -> None:
        self._uc: Uc | None = Uc(*self._UC[cpu_arch])
    ...
    def _require_uc(self) -> Uc:
        if self._uc is None:
            raise RuntimeError("Unicorn backend is closed")
        return self._uc

    def reg_read(self, reg_id: int) -> int:
        return self._require_uc().reg_read(reg_id)

    def reg_write(self, reg_id: int, value: int) -> None:
        self._require_uc().reg_write(reg_id, value)

    def read_sysreg(self, op0: int, op1: int, crn: int, crm: int, op2: int) -> int:
        self._require_arm64()
        return self._require_uc().reg_read(UC_ARM64_REG_CP_REG, (crn, crm, op0, op1, op2))

    def write_sysreg(self, op0: int, op1: int, crn: int, crm: int, op2: int, value: int) -> None:
        self._require_arm64()
        self._require_uc().reg_write(UC_ARM64_REG_CP_REG, (crn, crm, op0, op1, op2, value))
    ...
    def mem_map(self, address: int, size: int, perms: MemoryProtectionFlag) -> None:
        try:
            self._require_uc().mem_map(address, size, int(perms))
        except UcError as e:
    ...
    def mem_protect(self, address: int, size: int, perms: MemoryProtectionFlag) -> None:
        try:
            self._require_uc().mem_protect(address, size, int(perms))
        except UcError as e:
    ...
    def hook_add(self, hook_type: HookType, callback: Callable[..., object], begin: int = 1, end: int = 0) -> int:
        return self._require_uc().hook_add(self._HOOK_IDS[hook_type], callback, begin=begin, end=end)

    def hook_del(self, handle: int) -> None:
        self._require_uc().hook_del(handle)

    def flush_tb(self) -> None:
        self._require_uc().ctl_flush_tb()  # type: ignore[attr-defined]

    def emu_start(self, begin: int, until: int, timeout: int = 0, count: int = 0) -> None:
        try:
            uc = self._require_uc()
            uc.emu_start(begin, until, timeout, count)
        except UcError as e:
            pc = uc.reg_read(self._pc_reg)
            raise EmulatorCrashed(f"guest fault at pc={pc:#x}: {e}") from e

    def emu_stop(self) -> None:
        self._require_uc().emu_stop()

    def context_save(self) -> UcContext:
        return self._require_uc().context_save()

    def context_restore(self, context: object) -> None:
        if not isinstance(context, UcContext):
            raise TypeError(f"expected a Unicorn context, got {type(context).__name__}")
        self._require_uc().context_restore(context)
